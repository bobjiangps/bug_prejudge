...
class MLPrejudgeHelper:
        ...
        if len(init_test_round_results) > 1:
            ...
            for seq in range(len(not_errors)):
                ...
                if script_result_id not in script_result.keys():
                    script_result[script_result_id] = {"result": case_prejudge_result, "cases": {str(case.automation_case_result_id[index]): case_prejudge_result}}
                else:
                    script_result[script_result_id]["cases"][str(case.automation_case_result_id[index])] = case_prejudge_result
    ...
    def neighbor_classifier(cls, init_triage_history, init_test_round_errors, neighbor=3):
        ...
        # for seq in range(len(test_round_results)):
        #     print(seq)
        #     print(knn.predict([test_round_results.iloc[seq, [i for i in range(12)]]]))
        #     print(knn.predict_proba([test_round_results.iloc[seq, [i for i in range(12)]]]))
